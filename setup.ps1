# ==============================================================================
# WINDOWS AUTOMATED SETUP V25.1 - OPTIMIZED FOR dungbaminh/win_autosetup
# ==============================================================================
...
if ($MyInvocation.MyCommand.CommandType -ne "ExternalScript") {
    try {
        ...
        Write-Error "Loi ket noi GitHub. Vui long kiem tra Internet!"
        break
    }
...
# --- STAGE 4: OFFICE, APPS & TWEAKS ---
if ($currentStage -eq "4") {
    Write-Host "`n[STAGE 4] TOI UU & CAI DAT PHAN MEM..." -ForegroundColor Cyan
    ...
    # 2. Office 2024 (Doi cho den khi cai xong)
    Write-Host "Dang cai Office 2024 (Word, Excel, PPT, OneDrive)..." -ForegroundColor Yellow
    irm https://get.massgrave.dev | iex /o-ltsc2024 /apps:word,excel,powerpoint,onedrive /s
    Write-Host "Dang kich hoat Office..." -ForegroundColor Yellow
    irm https://get.massgrave.dev | iex /ohook

    # 3. Cai Apps
    $apps = @("Google.Chrome", "PhamKimLong.UniKey", "RARLab.WinRAR", "7zip.7zip", "Zalo.Zalo")
    foreach ($a in $apps) { 
        Write-Host "Dang cai: $a" -ForegroundColor Green
        winget install --id $a --silent --non-interactive --accept-package-agreements --accept-source-agreements 
    }
    ...
    # 6. Restart Explorer de ap dung Menu chuot phai ngay lap tuc
    Stop-Process -Name explorer -Force -ErrorAction SilentlyContinue

    Remove-Autostart
    Write-Host "`n=== HOAN THANH! HE THONG DA SN SANG. ===" -ForegroundColor Green
    Start-Sleep -Seconds 5
    Restart-Computer -Force
}
